...
\end{table}
       ...
       \Note{Remember that area is measured in square units: e.g. cm$^2$, m$^2$ or mm$^2$.}     
...
\Definition{Right prism}{A right prism is a geometric solid that has a polygon as its base and one or more sides perpendicular to the base. The base and top surface are the same shape and size. It is called a ``right`` prism because the angles between the base and sides are right-angles.}
...
\Definition{Surface area}{Surface area is the total area of the exposed or outer surfaces of a prism.}
...
\Note{Remember that the volume is measured in cubic units: e.g. cm$^3$, m$^3$ or mm$^3$.}
...
\Definition{Pyramid}{A pyramid is a geometric solid that has a polygon base and sides that converge at a point called the apex (in other words the sides are \textbf{not} perpendicular to the base}. 
...
\psdots[dotsize=0.09](1.92,-0.16)
% \usefont{T1}{ptm}{m}{n}
\rput(2.7095313,0.12){\psframebox[linewidth=0.002,linecolor=color129b,fillstyle=solid,fillcolor=color129b]{$5$ cm}}
\end{pspicture} 
}
...
{
\westep{Calcuate the volume of the object}
...
Up until now, we have examined the effect of multiplying with a constant factor of integer value. We can also express
...
\begin{enumerate}[noitemsep, label=\textbf{\arabic*}. ] 
\setcounter{enumi}{6}
\item Calculate the volume and surface area of the solid below (correct to one decimal place):
\end{enumerate}
\begin{center}
\scalebox{1} % Change this value to rescale the drawing.
{
...
% \usefont{T1}{ppl}{m}{n}
\rput(3.596718,-0.44921875){$50$ cm}
