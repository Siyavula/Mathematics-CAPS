% NOTE: There are a couple of \clearpage commands that I used to make
...
\textbf{Experiment 1} A coin is tossed and it lands with either
  heads (H) or tails (T) facing upwards. An example outcome of tossing a coin that lands with heads
    facing up:
  \begin{center}
    \begin{tikzpicture}
      \coinheads
...
\mindsetvid{venn diagrams and probabilities}{VMcbf}
...
\begin{wex}{Venn diagrams}{
  Represent the sample space of two rolled dice and the two events, $A$ and $B$, using a Venn diagram, where
  \begin{itemize}
...
\begin{wex}{Union and intersection of events}{
  Relate the probabilities of events $A$ and $B$ from the previous worked example (two rolled dice) and show that they satisfy the identity
  \[P(A \cup B) = P(A) + P(B) - P(A \cap B)\]
}{
  \westep{Write down the probabilities of the two events, their union
    and their intersection.}
  
  From the Venn diagram in the previous worked example,
% Figure \ref{fig:dice_venn}
 we can count the number of outcomes in each
  ...
  \[\begin{array}{r@{$\ =\ $}c@{$\ =\ $}l}
    ...
    P(B)        & \dfrac{n(A)}{n(S)}        & \dfrac{11}{36} \\[8pt]
...
the middle column of the Venn diagrams provided in 'Union and intersection'.

% Figure \ref{fig:venn_union_intersection}.

\begin{figure}[H]
  \input{Gr10-Probability-images/venn_union_intersection3}
  \begin{center}\begin{caption*}{Mutually exclusive events.}\end{caption*}\end{center}
\end{figure}

From the above figure you can see that the intersection has no
...
\end{wex}
% \clearpage %manual clearpage to fix broken mdframed for Definition below
...
\begin{wex}{Reasoning with Venn diagrams}{
  ...
  \item We are not told how many people use both products, so we have
    ...
    \begin{equation*}
     \begin{array}{rcl}
       P(A \cup B) & = & P(A) + P(B) - P(A \cap B) \vspace{8pt}\\
      \dfrac{n(A \cup B)}{n(S)} & = & \dfrac{n(A)}{n(S)} + \frac{n(B)}{n(S)} - \dfrac{n(A \cap B)}{n(S)}\vspace{8pt} \\
      \dfrac{55}{70} & = & \dfrac{25}{70} + \dfrac{35}{70} - \dfrac{n(A \cap B)}{70}\vspace{8pt}  \\
...
\itemThe sample space of an experiment is
  the set of all possible outcomes of that experiment. The sample
  space is denoted with the symbol $(S)$ and the size of the sample
  space (the total number of possible outcomes) is denoted with
  $(n(S))$.
...
\item Probabilities of complementary events: $P(A) + P(A') = P(A \cup A') = P(S) = 1$
