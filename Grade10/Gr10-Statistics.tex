...
\begin{wex}{Median for an odd number of values}{
  ...
  namely, in the $4^{\mbox{th}}$ position. The value in the $4^{\mbox{th}}$ position is
...
\begin{wex}{Median for an even number of values}{
  ...
  middle, namely, between the $4^{\mbox{th}}$ and $5^{\mbox{th}}$ positions. The value in the
  $4^{\mbox{th}}$ position is $11$ and the value in the $5^{\mbox{th}}$ position is
...
\begin{wex}{Effect of outliers on mean and median}{
    ...
    and after the $11^{\mbox{th}}$ learner was included.
}{
  ...
  lies halfway between the $5^{\mbox{th}}$ and $6^{\mbox{th}}$ values:
  \begin{equation*}
  ...
  \end{equation*}
  Now, with $11$ values, the median is the $6^{\mbox{th}}$ value: $157$ cm.
...
\Definition{Percentile}{The $p^{\mbox{th}}$ percentile is the value, $v$, that
...
\par
 ...
 As an example, $13,0$ is at the $40^{\mbox{th}}$ percentile since there are
...
In general, the formula for finding the $p^{\mbox{th}}$ percentile in an ordered
data set with $n$ values is
\begin{equation*}
  r = \frac{p}{100}\left(n-1\right)+1
\end{equation*}
This gives us the rank, $r$, of the $p^{\mbox{th}}$ percentile. To find the
value of the $p^{\mbox{th}}$ percentile, we have to count from the first value in
the ordered data set up to the $r^{\mbox{th}}$ value.
...
$50\% = \frac{1}{2}$, the median is exactly the same as the $50^{\mbox{th}}$
...
equivalent to the $0^{\mbox{th}}$ percentile. Similarly, the maximum is
equivalent to the $100^{\mbox{th}}$ percentile.
...
\begin{wex}{Using the percentile formula}{
    ...
    gives the same answer. The minimum is equivalent to the $0^{\mbox{th}}$
    percentile. According to the percentile formula the rank, $r$, of the
    $p = 0^{\mbox{th}}$ percentile in a data set with $n=9$ values is
    \begin{align*}
      r &= \frac{p}{100}\left(n-1\right)+1 \\
    ...
    ordered data set. The maximum is also equivalent to the $100^{\mbox{th}}$
    ...
    \begin{align*}
      r &= \frac{p}{100}\left(n-1\right)+1 \\
        ...
        &= 9
    \end{align*}
    This confirms that the maximum value is the last (the $9^{\mbox{th}}$) value
    in the list, namely $45$.

    \westep{Find the median}

    The median is equivalent to the $50^{\mbox{th}}$ percentile. Using the
    ...
    \begin{align*}
      r &= \frac{50}{100}\left(n-1\right)+1 \\
        ...
        &= 5
    \end{align*}
    This shows that the median is in the middle (at the $5^{\mbox{th}}$ position)
...
\Definition{Quartiles}{The quartiles are the three data values that
  ...
  contains an equal number of data values. The median ($50^{\mbox{th}}$
  percentile) is the second quartile ($Q2$). The $25^{\mbox{th}}$ percentile is also
  be called the first or lower quartile ($Q1$), and the $75^{\mbox{th}}$ percentile the third or upper
  quartile ($Q3$).}
...
\begin{wex}{Quartiles}{
    ...
    Using the percentile formula, $n=12$, we can find the rank of the $25^{\mbox{th}}$,
    $50^{\mbox{th}}$ and $75^{\mbox{th}}$ percentiles as
    \begin{align*}
    ...
    For the $25^{\mbox{th}}$ percentile the rank is $3,75$, which is between
    the $3^{\mbox{rd}}$ and $4^{\mbox{th}}$ values. Since both these values are equal to
    $7$, the $25^{\mbox{th}}$ percentile is $7$.

    For the $50^{\mbox{th}}$ percentile (the median) the rank is $6,5$, meaning
    halfway between the $6^{\mbox{th}}$ and $7^{\mbox{th}}$ values. The $6^{\mbox{th}}$ value is
    $11$ and the $7^{\mbox{th}}$ value is $12$, which means that the median is
    \(\frac{11+12}{2} = 11,5\).

    For the $75^{\mbox{th}}$ percentile the rank is $9,25$, meaning between the
    $9^{\mbox{th}}$ and $10^{\mbox{th}}$ values. Therefore the $75^{\mbox{th}}$ percentile is
    \(\frac{31+35}{2} = 33\).
  }
\end{wex}
...
\vspace {8pt}\\
    ...
    \item In which interval is the $30^{\mbox{th}}$ percentile of the data set?
    \end{enumerate}
}{
 ...
 \mbox{Mean } &=  \frac{
  ...
  $100$ learners, we can find the rank of the $25^{\mbox{th}}$, $50^{\mbox{th}}$ and
  $75^{\mbox{th}}$ percentiles as
  \begin{align*}
  ...
  \item For the second quartile (the median), we have that there are
    ...
    lies somewhere in the fifth range: $50 \le x < 60$.
  \item For the upper quartile, we have that there are $65$ learners
    ...
    lies somewhere in the sixth range: $60 \le x < 70$.
  \end{itemize}

  \westep{Find the $30^{\mbox{th}}$ percentile}

  Using the same method as for the quartiles, we first find the rank
  of the $30^{\mbox{th}}$ percentile.
  \begin{align*}
  ...
  in the first $4$ ranges combined, the $30^{\mbox{th}}$ percentile lies in the
...
$100^{\mbox{th}}$ and the $0^{\mbox{th}}$ percentile (that is, between the maximum and
...
five number summary consists of the following percentiles: $0^{\mbox{th}}$,
$25^{\mbox{th}}$, $50^{\mbox{th}}$, $75^{\mbox{th}}$, $100^{\mbox{th}}$.
...
\begin{figure}[t]
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw (3.5, 2.5) node[anchor=west] {($100^{\mbox{th}}$ percentile)};
      \draw (3.5, 1) node[anchor=west] {($75^{\mbox{th}}$ percentile)};
      \draw (3.5, 0) node[anchor=west] {($50^{\mbox{th}}$ percentile)};
      \draw (3.5, -1) node[anchor=west] {($25^{\mbox{th}}$ percentile)};
      \draw (3.5, -2.5) node[anchor=west] {($0^{\mbox{th}}$ percentile)};
...
\begin{wex}{Five number summary}{
  ...
  we can determine that the median lies between the $6^{\mbox{th}}$ and $7^{\mbox{th}}$
  values, making it
  \begin{equation*}
    \frac{3,2 + 4,1}{2} = 3,65
  \end{equation*}

  The first quartile lies between the $3^{\mbox{rd}}$ and $4^{\mbox{th}}$ values, making
  it
  \begin{equation*}
    \frac{2,5 + 2,5}{2} = 2,5
  \end{equation*}

  The third quartile lies between the $9^{\mbox{th}}$ and $10^{\mbox{th}}$ values, making
  it
  \begin{equation*}
...
\item Lisa is working in a computer store. She sells the following
  ...
  \end{equation*}
  Provide the five number summary and box-and-whisker plot of Lisa's
  sales.
...
\item Hannah has worked as a florist for nine months. She sold the
  following number of wedding bouquets each month:
  \begin{equation*}
...
\item The $p^{\mbox{th}}$ percentile is the value, $v$, that divides a data set into two parts, such that 
$p\%$ of the values in the data set are less than $v$ and $100 − p\%$ of the 
values are greater than $v$. 

The general formula for finding the $p^{\mbox{th}}$ percentile in an ordered data set with $n$ values is 
\begin{equation*}
  r = \frac{p}{100}\left(n-1\right)+1
\end{equation*}
...
\begin{eocexercises}{}
  ...
  \item An engineering company has designed two different types of
    ...
    \begin{center}
      \begin{tabular}{|c@{\hspace{0.1cm}}|@{\hspace{0.1cm}}c@{\hspace{0.1cm}}|@{\hspace{0.1cm}}c@{\hspace{0.1cm}}|@{\hspace{0.1cm}}c@{\hspace{0.1cm}}|@{\hspace{0.1cm}}c@{\hspace{0.1cm}}|@{\hspace{0.1cm}}c@{\hspace{0.1cm}}|@{\hspace{0.1cm}}c@{\hspace{0.1cm}}|@{\hspace{0.1cm}}c@{\hspace{0.1cm}}|@{\hspace{0.1cm}}c@{\hspace{0.1cm}}|@{\hspace{0.1cm}}c@{\hspace{0.1cm}}|@{\hspace{0.1cm}}c|} \hline
