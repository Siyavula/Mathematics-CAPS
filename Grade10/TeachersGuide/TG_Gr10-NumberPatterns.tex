...
50 = 5n \\
n = 10 $ weeks
\item With one line intersecting at four points we get five parts. If we add a seconde line it is now broken up into 9 parts. And if we add a third line it is now broken up into 13 parts. So we see that for each line added we add four parts. The sequence is: $5 ; 9 ; 13 \ldots$\\
$a = 5\\
d = 4\\
T_n = 5 + 4(n - 1)\\
T_n = 4n + 1  \\
...
\item 
  \begin{enumerate}[noitemsep, label=\textbf{(\alph*)} ]
  \item The first digit of the answer increases by one and the second digit decreases by one.
  \item When 9 is added to any 2 digit number the answer is the number with the first digit increased by one and the second digit decreased by one.
