...
\section[Drawing figures on the Cartesian plane]{\Huge Drawing figures on the Cartesian plane}
...
\mindsetvid{Distance formula}{VMbls}
...
\uput[dr](2,-1.8){\Large{$C(x_{2};y_{1})$}}
...
\Identity{\begin{center}Distance: $d=\sqrt{{({x}_{1}-{x}_{2})}^{2}+{({y}_{1}-{y}_{2})}^{2}}$\end{center}}
\Note{Note that $(x_1 - x_2)^2 = (x_2 - x_1)^2$.}
...
\begin{wex}{Using the distance formula}{Find the distance between $S(-2;-5)$ and $T(7;-2)$.}{
...
\begin{wex}{Using the distance formula}
{Given $RS = 13$, $R(3;9)$ and $S(8;t)$, find $t$.}
{
\westep{Draw a sketch}
...
\usefont{T1}{ppl}{m}{n}
\rput(4.9,3.1832812){$S_2(8;t_2)$}
\usefont{T1}{ppl}{m}{n}
\rput(4.9,-3.0067186){$S_1(8;t_1)$}
...
\end{center}
From the sketch we see that there are $2$ solutions for $t$.

\westep{Assign values to $(x_1;y_1)$ and $(x_2;y_2)$}
Let the coordinates of $R$ be $(x_1;y_1)$ and the coordinates of $S$ be $(x_2;y_2)$.
\begin{equation*}
x_1 = 3 \hskip2em y_1 = 9 \hskip2em x_2 = 8 \hskip2em y_2 = t
\end{equation*}
\westep{Write down the distance formula}
\begin{equation*}
...
\item Find the length of $AB$ for:
...
\item The length of $CD$ is $5$. Find the missing coordinate for:
 \begin{enumerate}[noitemsep, label=\textbf{(\alph*)} ] 
...
\Definition{Gradient}{The gradient between two points is determined by the ratio of vertical change to horizontal change.}
Gradient describes the slope or steepness of the line joining two points. In the figure below, line $OQ$ is the least steep and line $OT$ is the steepest.
...
\Identity{\begin{center}Gradient: $m = \dfrac{y_{2} - y_{1}}{x_{2} - x_{1}}$ \ or\ $\dfrac{y_{1} - y_{2}}{x_{1} - x_{2}}$\end{center}}
\Tip{Remember to be consistent: $m \neq \dfrac{y_{1} - y_{2}}{x_{2} - x_{1}}$}
\begin{wex}{Gradient between two points}
...
\end{equation*}
\westep{Write the final answer}
The gradient of $EF$ is $-\dfrac{4}{5}$.
}
\end{wex}


\begin{wex}{Gradient between two points}{Given $G(7;-9)$ and $H(k;0)$, with $m_{GH}= 3$, find $k$.}{
\westep{Draw a sketch}
\begin{center}
\scalebox{1} % Change this value to rescale the drawing.
{
...
\usefont{T1}{ppl}{m}{n}
\rput(5.5245314,0.6432812){$H(k;0)$}
\usefont{T1}{ppl}{m}{n}
...
\begin{equation*}
  x_1 = 7 \hskip2em y_1 = -9 \hskip2em x_2 = k \hskip2em y_2 = 0
\end{equation*}
...
\item If the gradient of $CD$ is $\frac{2}{3}$, find $p$ given
\begin{enumerate}[noitemsep, label=\textbf{(\alph*)} ] 
...
\Definition{Straight line}{A straight line is a set of points with a constant gradient between any pair of points.}
...
\end{center}
We have $m_{AB} = m_{BC}=m_{AC} = \dfrac{y_2-y_1}{x_2-x_1} = \dfrac{y_1-y_2}{x_1-x_2}$.
The general formula for finding the equation of a straight line is $\dfrac{y-y_1}{x-x_1} = \dfrac{y_2-y_1}{x_2-x_1}$ where $(x;y)$ is any point on the line. This formula can also be written as $y-y_1 = m(x-x_1)$.
...
\begin{wex}{Parallel lines}
{Prove that the line $AB$ with points $A(0;2)$ and $B(2;6)$ is
...
\westep{Write down the gradient formula}
\begin{equation*}
  m = \dfrac{y_2-y_1}{x_2-x_1}
\end{equation*}
...
\westep{Write down the formula for the product of the gradients of perpendicular lines}
\begin{align*}
  m_{AB} \times m_{CD} &= -1\\
...
Points are collinear if they lie on the same line. There are two
methods to prove that points are collinear: the gradient method and a
longer method using the distance formula.

If three points are collinear then the line joining them will have the
same gradient at any point along the line.  Therefore to prove
collinearity, we must prove that two of the gradients between any of
the three points are equal.

\begin{wex}{Collinear points}
{Prove that $A(-3;3)$, $B(0;5)$ and $C(3;7)$ are collinear.}{
\westep{Draw a sketch}
\begin{center}
\scalebox{1} % Change this value to rescale the drawing.
{
...
\psline[linewidth=1pt,linestyle=solid,dash=0.16cm 0.16cm,arrowsize=0.05291667cm 2.0,arrowlength=1.4,arrowinset=0.4]{<->}(1.65,-0.12671874)(2.93,0.7732813)
\psline[linewidth=1pt,linestyle=solid,dash=0.16cm 0.16cm,arrowsize=0.05291667cm 2.0,arrowlength=1.4,arrowinset=0.4]{<->}(3.09,0.91328126)(4.39,1.8132813)
\psline[linewidth=1pt,linestyle=solid,dash=0.16cm 0.16cm,arrowsize=0.05291667cm 2.0,arrowlength=1.4,arrowinset=0.4]{<->}(1.79,-0.42671874)(4.49,1.5332812)
\end{pspicture} 
}
...
\begin{exercises}{}{
    ...
    \item Determine whether $AB$ and $CD$ are parallel, perpendicular or neither for:
      \begin{enumerate}[noitemsep, label=\textbf{(\alph*)} ]
...
\end{activity}
\mindsetvid{Midpoint of a line segment}{VMbpr}
...
\Identity{\begin{center}Midpoint: $M(x;y)=\left(\dfrac{{x}_{1} + {x}_{2}}{2};\dfrac{{y}_{1}+{y}_{2}}{2}\right) $\end{center}}
...
\westep{Assign values to $(x_1;y_1)$ and $(x_2;y_2)$}
\begin{equation*}
  x_1 = -2 \hskip2em y_1 = -2 \hskip2em x_2 = 2 \hskip2em y_2 = 1
\end{equation*}
\westep{Write down the mid-point formula}
\begin{equation*}
  F(x;y)=\left(\dfrac{{x}_{1} + {x}_{2}}{2};\dfrac{{y}_{1}+{y}_{2}}{2}\right)
\end{equation*}
...
& = & \sqrt{6,25}
\end{eqnarray*}
As expected, $FE=FG$, therefore $F$ is the mid-point between $E$ and $G$.
}
\end{wex}
...
%     \nopagebreak
%%%             \label{m39167} $ \hspace{-5pt}\begin{array}{cccccccccccc}   \end{array} $ \hspace{2 pt}\raisebox{-0.2em}{\includegraphics[height=1em]{../icons/www.pdf}}{(section shortcode: MG10113 )} \par 
%     
%     
% 
...
\begin{wex}{Using the mid-point formula}{The line joining $C(-2;4)$ and $D(a;b)$ has the mid-point $M(1;-3)$. Find point $D$.}{
\westep{Draw a sketch}
\begin{center}
\scalebox{1} % Change this value to rescale the drawing.
{
...
\begin{equation*}
x_1=-2 \hskip2em y_1=4 \hskip2em x_2=a \hskip2em y_2=b
\end{equation*}
\westep{Write down the mid-point formula}
\begin{equation*}
M(x;y) = \Big(\frac{x_1+x_2}{2}; \frac{y_1+y_2}{2}\Big)
...
\begin{align*}
  1          &= \frac{-2+a}{2} & -3         &= \frac{4+b}{2} \\
  1 \times 2 &= -2+a           & -3\times 2 &= 4+b \\
  2          &= -2+a           & -6         &= 4+b \\
  a          &= 2+2            & b          &= -6-4 \\
  a          &= 4              & b          &= -10
\end{align*}
...
\begin{wex}{Using the mid-point formula}{Points $E(-1;0)$, $F(0;3)$, $G(8;11)$ and $H(a;b)$ are points on the Cartesian plane. Find $H(a;b)$ if $EFGH$ is a parallelogram.}{
\westep{Draw a sketch}
\begin{center}
\scalebox{1} % Change this value to rescale the drawing.
{
...
will be the same as the mid-point of $FH$. We first find the mid-point of $EG$. We can then use it to determine the coordinates of point $H$.
\westep{Assign values to $(x_1;y_1)$ and $(x_2;y_2)$}
...
\begin{equation*}
 M(x;y) =\Big(\frac{x_1+x_2}{2}; \frac{y_1+y_2}{2}\Big)
\end{equation*}
\westep{Substitute values and calculate the coordinates of $M$}
\begin{equation*}
...
$M(\frac{7}{2};\frac{11}{2})$ and $F(0;3)$ to solve for $H(a;b)$
\westep{Substitute values and solve for $a$ and $b$}

\begin{equation*}
\begin{array}{rllrl}
 M\Big(\dfrac{7}{2};\dfrac{11}{2}\Big) &=~\Big(\dfrac{x_1+x_2}{2}; \dfrac{y_1+y_2}{2}\Big)\\[8pt]
\dfrac{7}{2}&=~\dfrac{0+a}{2}&\hskip10em&\dfrac{11}{2}&=~\dfrac{3+b}{2}\\
7&=~a+0&&11&=~3+b\\
a&=~7&&b&=~8\\
\end{array}
\end{equation*}
...
\end{equation*}
\item A straight line is a set of points with a constant gradient
  between any pair of points from the set.
