...
\end{array}
\end{equation*}
\label{m39241*id145267}Solving an equation means finding find the value of the variable that makes
...
\label{m39241*id145297}For linear equations, there is at most one solution for the equation.\par 
\label{m39241*id145312}To solve equations we use algebraic methods that include expanding expressions, grouping terms and factorising.\par 
...
\end{equation*}
\label{m39241*id145475}Check the answer by substituting $x=-\frac{1}{2}$, back into the original equation:
...
\begin{equation*}
    ...
    \end{array}
\end{equation*}
\label{m39241*id145603}Therefore $x = -\frac{1}{2}$ is a valid solution.
...
\label{m39241*id145627}The general steps for solving linear equations are:
...
\begin{equation*}
    \begin{array}{ccc}\hfill 4(2x-9)-4x& =& 4-6x\hfill  \\ 
...
Division by $0$ is not permitted, therefore there must be a restriction on the value of $x$. ($x\neq \frac{-1}{3}$)
...
\end{array}
\end{equation*}
\label{m39241*id147513}Since both sides are equal, the answer is correct.
}
\end{wex}
...
\westep{Check answer by substituting solution back into original equation}
\begin{equation*}
    \begin{array}{cccc}\hfill \dfrac{2(-\frac{3}{8}) - 3}{3} - 3(-\frac{3}{8}) &= & \dfrac{-\frac{3}{8}}{3}\hfill & \\ 
\\
...
\label{m39247*id149356}A quadratic equation is an equation where the exponent of the variable is at most
...
\label{m39247*id149478}Quadratic equations differ from linear equations in that a linear
...
\label{m39247*id149489}We solve quadratic equations using factorisation. For example in order to solve $2{x}^{2}-x-3 = 0$
...
\\For example, $2{x}^{2}+4x+2=0$\hspace{1ex} can be written as
...
{
\westep{The equation is in the required form}
...
The solution to $3{x}^{2}+2x-1=0$\hspace{1ex} is $x=-1$ or $x=\frac{1}{3}$.
}
\end{wex}
...
\label{m39247*id153262}The solution to $0=-2{x}^{2}+4x-2$ is $x=1$.}
}
\end{wex}
...
{
Solve the following system of equations:\par 
\begin{equation*}
\begin{array}{ccc}\hfill x+y& =& 1\hfill \\ \hfill 3& =& y-2x\hfill \end{array}
\end{equation*}
}
{
...
\begin{equation*}
    \begin{array}{ccc}\hfill x - (-5)& =& 1\hfill \\
	\hfill x+ 5& =& 1\hfill \\
...
\westep{Write the final answer}
\begin{equation*}
\begin{array}{ccc}\hfill y& =& -5\hfill \\
 \hfill x& =& -4\hfill 
\end{array}
\end{equation*}
}
...
\hfill 4y - 19x& =& 12\hfill \end{array}
\end{equation*}
}
{
...
Solve the following system of equations:
\begin{equation*}
\begin{array}{cccc}\hfill & 3x+y& =& 2\hfill \\ 
...
\nopagebreak
\label{m39257*id159067}Simultaneous equations can also be solved graphically. If the graphs of each linear equation are drawn, then the solution to the system of simultaneous equations is the co-ordinate of the point at which the two graphs intersect.\par 
For example:
\begin{equation*}
\begin{array}{cc}\hfill x=2y\\ \hfill y=2x-3\end{array}
\end{equation*}

\label{m39257*id159120}The graphs of the two equations are shown below.\par 
...
\end{center}
\end{figure}       
\label{m39257*id159137}The intersection of the two graphs is $(2;1)$. So the solution to the system of simultaneous equations is $x=2$ and $y=1$.\par 
\label{m39257*id159194}We can also check the solution using algebraic methods. \\
...
\label{m39257*id159690}The two graphs intersect at $(4;22)$. 
...
\label{m39262*uid104}\item Read the whole the question
\label{m39262*uid105}\item What are we asked to solve for?
\label{m39262*uid106}\item Assign a variable to the unknown quantity e.g., $x$.
\label{m39262*uid107}\item  Translate the words into algebraic expressions by rewriting the given information in terms of the variable. 
\label{m39262*uid108}\item Set up an equation or system of equations to solve for the variable.
\label{m39262*uid109}\item Solve the equation algebraically using substitution.
...
We have two unknowns, Bongani's mark and
 Jane's mark. Let Bongani's mark be $t$ and Jane's mark be $j$. 
...
\label{m39262*id161608}Also, both marks add up to $14$.
...
{
Three rulers and two pens have a total cost of R $21,00$. One ruler and one pen have a total cost of R $8,00$. How much does a ruler cost and how much does a pen cost? 
} 
{
\westep{Assign variables to unknowns}
Let the cost of one ruler be $x$ 
\\and let the cost of one pen be $y$.\par 

\westep{Set up a system of equations}  
\begin{equation*}
\begin{array}{ccl}\hfill 3x+2y& =& 21\hfill \\ \hfill x+y& =& 8\hfill \end{array}
\end{equation*}

\westep{Use the second equation to express $y$ in terms of $x$}

\begin{equation*}
y=8-x
\end{equation*}

\westep{Substitute into first equation}

\begin{equation*}
    \begin{array}{ccl}\hfill 3x+2(8-x)& =& 21\hfill \\
    \end{array}
\end{equation*}
...
\begin{equation*}
	...
	\hfill \therefore  x& =& 5\hfill 
    \end{array}
\end{equation*}

\westep{Substitute value for $x$ back into second equation and solve for $y$}

\begin{equation*}
\begin{array}{ccl}\hfill y& =& 8-5\hfill \\ \hfill \therefore y& =& 3\hfill \end{array}
\end{equation*}
\westep{Check the solution satisfies both original equations}
\westep{Write final answer} 
...
{
\westep{Assign variables to unknowns}
\label{m39257*id160923}Let $b$ be the number of bicycles \\
and let $t$ be the number of tricycles. 
...
\westep{Use the first equation to express $b$ in terms of $t$}
\begin{equation*}
\begin{array}{ccc}\hfill b& =& 7-t\hfill \end{array}
\end{equation*}

\westep{Substitute into the second equation}
\begin{equation*}
\begin{array}{ccc} \hfill 2(7-t)+3t & =& 19\hfill 
\end{array}
\end{equation*}

\westep{Rearrange and solve for $t$}
\begin{equation*}
\begin{array}{ccc}
 \hfill 14-2t+3t& =& 19\hfill \\
 \hfill \therefore t& =& 5\hfill 
\end{array}
\end{equation*}

\westep{Substitute value for $t$ back into second equation and solve for $b$}
\begin{equation*}
\begin{array}{ccc}
\\ \hfill  b & =& 7-5\hfill \\ \therefore b& =& 2
\end{array}
\end{equation*}
 \westep{Check the solution satisfies both original equations}
\westep{Write final answer}
...
\westep{Alternative solution}
This example can also be solved using only one variable:\\
Let the number of bicycles be $x$ and the number of tricycles sold $7-x$.\\
The total number of wheels is $19$ so:
\begin{equation*}
\begin{array}{ccc}
\hfill  2(x)+3(7-x) & =& 19\hfill \\ 
\hfill 2x + 21 - 3x &=& 19 \hfill\\
\hfill -x &=& -2\hfill \\
\hfill \therefore x &=& 2 
\end{array}
\end{equation*}
Therefore $2$ bicycles and $7-2=5$ tricycles were sold.
...
{
\westep{Assign variables to unknowns}  
Let the price of a chocolate milkshake be $x$ and the price of a fruitshake be $y$.
...
{Solving literal equations}
{
Given the formula $e=\dfrac{E}{R+r} \times R$, solve for $R$.
}
{
\westep{Isolate the required variable}
\begin{equation*}
    \begin{array}{ccll}\hfill e(R+r)& =& E \times R\hfill & \\
	\hfill eR + er& =& ER\hfill & \hfill \\
	\hfill eR & =& ER - eR\hfill \\
\hfill eR & =& R(E - e)\hfill \vspace{5pt}\\ 
\hfill \therfore R & =& \dfrac{er}{E-e}\hfill 
    \end{array}
\end{equation*}
...
\end{equation*}
\label{m39254*id157374}The methods used to solve linear inequalities are similar to those used to
...
\end{figure}       
\par 
\label{m39254*id157780}We see that for the equation there is only a single value of $x$ for which the equation is true. However, for the inequality, there is a range of values for which the inequality is true. This is the main difference between an equation and an inequality.\par 
...
\begin{exercises}{ }
{
Solve for $x$ and represent the answer on a number line:
\begin{enumerate}[noitemsep, label=\textbf{\arabic*}. ] 
