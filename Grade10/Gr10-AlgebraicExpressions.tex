...
\end{equation*}
We see that all denominators and all numerators are integers.\par 
...
\item decimal numbers that have a recurring pattern of numbers, for example the fraction $\dfrac{1}{3}$ can be written as 
$0,\dot{3}$. 
...
Find the two consecutive integers such that $\sqrt{26}$ lies between them.\par 
...
A binomial is a mathematical expression with two terms, e.g. $ax+b$ and $cx+d$. If these two binomials are multiplied (or expanded) we get the following:
\begin{equation*}
...
\Tip{If the binomial is \begin{math}A+B\end{math} and the trinomial is \begin{math}C+D+E\end{math}, then the very first step is to apply the distributive law:\par 
\begin{equation*}
...
\begin{Large}
$2(x+1)$ \begin{Huge} $leftharpoons$ \end{Huge} $2x+2$
\end{Large}\\
...
Therefore,
\begin{equation*}
{a}^{2}{x}^{2}-{b}^{2}~\mbox{ can be factorise\hspace{1ex}d as }~(ax+b)(ax-b)
\end{equation*}
...
\begin{wex}{}
{Factorise completely: $3a(a^2-4)-7(a^2-4)$ \par }
...
\Tip{
\begin{itemize}[itemsep=5pt]
\item If $c$ is positive, then the factors of $c$ must be either both positive or both negative. If $c$ is negative, it means only one of the factors of $c$ is negative, the other one being positive.
item Once you get an answer, always multiply out your brackets again just to make sure it really works.
\end{itemize}
...
2{x}^{2}+2x+3x+3
\end{equation*}
there is no common factor to all four terms, but we can factorise as follows:\par 
\nopagebreak\noindent{}
...
\end{equation*}}
\westep{Write the final answer}{
\label{m39387*id273290}The product of $x+y$ and ${x}^{2}-xy+{y}^{2}$ is ${x}^{3}+{y}^{3}$. \\
...
\In some cases of simplifying an algebraic expression, the expression will be a fraction. For example,\par 
\
\begin{equation*}
\dfrac{{x}^{2}+3x}{x+3}
\end{equation*}
...
\end{equation*}
If $x = 3$ then the denominator, $x-3 = 0$, and the fraction is undefined.\par 
