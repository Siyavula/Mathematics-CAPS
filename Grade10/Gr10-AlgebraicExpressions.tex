...
Factorise completely:7x+14y+bx+2by
\begin{multicols}{2}
\begin{enumerate}[itemsep=5pt, label=\textbf{\arabic*}. ] 
\setcounter{enumi}{16}
\item $3{x}^{2}+17x-6$
...
\end{equation}
\label{m39394*id278591}We get this by taking out the $(x+1)$ and seeing what is left over. We have a $+2x$\hspace{1ex}from the first group and a $+3$ from the second group. This is called \textsl{factorising by grouping}.\par 
...
\begin{wex}{Factorisation by grouping }{Find the factors of $7x+14y+bx+2by$}
{
...
\end{equation}
\westep{}
\label{m39394*id278811}Take out common factor $(x+2y)$
...
There is sometimes more than one correct way to group terms, as shown in the example below.
...
\westep{}\label{m39394*id278721}$x$ is a common factor of the first and third terms and $2y$ is a common factor of the second and fourth terms.\par 
...
\end{equation}
\westep{}
\label{m39394*id278811}Take out common factor $(7+b)$
...
\begin{exercises}{Factorising by grouping }{
\nopagebreak
Factorise the following`:
...
We now look at some special examples of finding the product of a binomial and a trinomial. 
...
\begin{activity}{Investigating sum and difference of cubes}
...
\end{equation}
\label{m39387*id273451}This is known as a \textsl{sum of two cubes}. \\So $x^{3}+y^{3}$ has factors $(x+y)$ and $(x^{2} -xy+y^{2})$.\\
\\
We have also seen that:
\begin{equation*}
\left(x-y\right)\left({x}^{2}+xy+{y}^{2}\right)={x}^{3}{y}^{3}
\end{equation}
This is known as a \textsl{difference of two cubes}. \\So $x^{3}-y^{3}$ has factors $(x-y)$ and $(x^{2} +xy+y^{2})$.
} 
...
{
\westep{}{
Take out common factor $16$.
\begin{equation*}
\begin{array}{ccc}16y^{3} - 432\hfill & =& 16(y^{3} - 27})\hfill & \\
\end{array}
\end{equation}}
\westep{}{
Notice that $\sqrt[3]{y^{3}} = y$ and $\sqrt[3]{27} = 3$. This gives the terms in the first bracket.}
\westep{}{
Using inspection we can factorise the difference of two cubes as 
\begin{equation*}
...
\westep{}{
Using inspection we can factorise the sum of two cubes as 
\label{m39387*id272771}\nopagebreak\noindent{}
\begin{equation*}
...
\end{equation}}
\westep{}{
\label{m39387*id273290}Expand the brackets to check that the expression has been correctly factorised. }
}
\end{wex}
...
\begin{exercises}{Factorising sum and difference of two cubes}
{
...
\westep{}Use grouping to factorise the numerator and take out common factor $ax$ in the denominator.\par 
...
\end{equation}
\westep{}  
\label{m39392*id279695}$(x -b)$ is a factor in the numerator and the denominator that can be cancelled. 
\begin{equation*}
\begin{array}{lll}
...
\end{equation}
\westep{} \label{m39392*id79242}Make all the denominators the same so that we can add or subtract the fractions. The lowest common denominator is $\left(x-2\right)\left(x+2\right)$.\par 
...
\end{equation}
\westep{} \label{m39392*id79242}Simplify and find common denominator.
...
\begin{exercises}{Simplifying fractions}
{
...
\label{m38349*uid22}\item Write the following irrational numbers to $3$ decimal places:
...
\label{m38349*uid27}\item Use your calculator and write the following irrational numbers to $4$ decimal places:
...
\item Write the following irrational numbers to $3$ decimal places and then write each one as a rational number to get an approximation to the irrational number.\\ For example, $\sqrt{3}=1,73205\ldots$. To $3$ decimal places, $\sqrt{3}=1,732$. $1,732=1\dfrac{732}{1000}=1\dfrac{183}{250}$. Therefore, $\sqrt{3}$ is approximately $1\dfrac{183}{250}$.
...
\item Determine between which two integers the following irrational numbers lie, without using a calculator:
...
\item $\left(16-{x}^{4}\right)$
\item ${7x}^{2}-14x+7xy-14y$
\item ${y}^{2}-7y-30$
\item $1-x-{x}^{2}+{x}^{3}$
\item $-3\left(1-{p}^{2}\right)+p+1$
\item $x-x^{3} + y - y^{3}$
\item $x^{2} - 2x + 1 - y^{4}$
...
\item ${\left(a-2\right)}^{2}-a\left(a+4\right)$\item $\left(5a-4b\right)\left(25{a}^{2}+20\mathrm{ab}+16{b}^{2}\right)$\item $\left(2m-3\right)\left(4{m}^{2}+9\right)\left(2m+3\right)$\item $\left(a+2b-c\right)\left(a+2b+c\right)$
\item $\dfrac{{p}^{2}-{q}^{2}}{p}÷\dfrac{p+q}{{p}^{2}-\mathrm{pq}}$\item $\dfrac{2}{x}+\dfrac{x}{2}-\dfrac{2x}{3}$
