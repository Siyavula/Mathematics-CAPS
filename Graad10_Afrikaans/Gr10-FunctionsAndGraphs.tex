...
\westep{Bepaal die as-afsnitte}
Funskie $f(x)$ sny die asse by die oorsprong $(0;0)$. \\
...
\westep{Skryf die finale antwoord}
$a=-1$ en $q=1$, dus die vergelyking van die hiperbool is $y=\dfrac{-1}{x}+1$.
}
\end{wex}
...
%lots of english
 \item Skets die grafieke van die volgende trigonometriese funksies as
    ...
    \end{enumerate}
  \item As die algemene vergelykings $y=mx+c$, $y=ax^2+q$, $y=\frac{a}{x}+q$, $y=a~sin~x+q$, $y=a^x +q$ en $y=a~tan~x$ gegee is, bereken die spesifieke vergelykings vir elkeen van die volgende grafieke:\vspace{20pt}\\
    \begin{center}
      \begin{table}[H]
...
\item In die diagram word die skets van  $y=-3x^2+3$ en $y=-\frac{18}{x}$ getoon.\\
    ...
    \item Beskryf in woorde wat gebeur op punt $D$.
    \item Bereken die ko\"ordinate van $D$.
    \item Bepaal die vergelyking van die reguitlyn wat deur $C$ en $D$ pas.
  \end{enumerate}
%NEED DIAGRAM HERE
\item In die diagram is $f(\theta)=3~sin~\theta$ en $g(\theta)=-tan~\theta$ geskets.
    \begin{enumerate}[noitemsep, label=\textbf{(\alph*)} ]
